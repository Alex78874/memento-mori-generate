// Set up vars
const myBirthDay = window.myBirthDay || '2003-12-15';
const myLifeExpectancy = window.myLifeExpectancy || 80;
const totalWeeksInLife = window.totalWeeksInLife || myLifeExpectancy * 52.1429;
...
progress = Math.ceil(totalWeeksInLife) - Math.ceil(weeksLeft) + ' semaines vécues. ' + weeksLeft + ' semaines restantes du total ' + Math.ceil(totalWeeksInLife) + ' semaines disponibles.';
...
function write_life_event(life_event) {
  ...
  if (week_div == null || week_div.classList.contains('invisible')) {
    ...
    console.error(`L'événement '${life_event['description']}' a une date invalide (${y}-${m}-${d})`);
    return;
  }
